...
def temporal_plots(output_dir, scenario_parameters, op_attributes, dir_names, op_id, show=False, evaluation_start_time = None, evaluation_end_time = None):
    op_stats = os.path.join(output_dir, "2-{}_op-stats.csv".format(int(op_id)))
    op_df = pd.read_csv(op_stats)
    if evaluation_start_time is not None:   # TODO # keep it this way?
...
def vehicle_stats_over_time(output_dir, scenario_parameters, operator_attributes, dir_names, op_id, show = True, bin_size = None, evaluation_start_time = None, evaluation_end_time = None):
...
def occupancy_over_time(output_dir, scenario_parameters, operator_attributes, dir_names, op_id, show = True, bin_size = None, evaluation_start_time = None, evaluation_end_time = None):
...
def occupancy_over_time_rel(output_dir, scenario_parameters, operator_attributes, dir_names, op_id, bin_size = None, show = True, evaluation_start_time = None, evaluation_end_time = None):
    """ this function creates a plot of the different vehicle occupancies over time """
    op_stats = os.path.join(output_dir, "2-{}_op-stats.csv".format(int(op_id)))
    op_df = pd.read_csv(op_stats)
    max_occupancy = op_df[op_df["status"] == "route"]["occupancy"].max()
...
def rq_stats_temporal(output_dir, bin_size = 900, evaluation_start_time = None, evaluation_end_time = None):
    sc_rq_df = pd.read_csv(os.path.join(output_dir, "1_user-stats.csv"))
    if evaluation_start_time is not None:
        sc_rq_df = sc_rq_df[sc_rq_df[G_RQ_TIME] >= evaluation_start_time]
    if evaluation_end_time is not None:
        sc_rq_df = sc_rq_df[sc_rq_df[G_RQ_TIME] <= evaluation_end_time]
...
def eval_day_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = None, evaluation_end_time = None):
    sc_user_df = pd.read_csv(os.path.join(output_dir, "1_user-stats.csv"))
    sc_user_df = sc_user_df[sc_user_df["vehicle_id"].notna()]
    if evaluation_start_time is not None:
        sc_user_df = sc_user_df[sc_user_df[G_RQ_TIME] >= evaluation_start_time]
    if evaluation_end_time is not None:
        sc_user_df = sc_user_df[sc_user_df[G_RQ_TIME] <= evaluation_end_time]
    sc_user_df = set_sc_rq_df_departure_delay_and_wt(sc_user_df)
    sc_op_df = pd.read_csv(os.path.join(output_dir, "2-0_op-stats.csv"))
    if evaluation_start_time is not None:   # TODO # keep it this way?
        sc_op_df = sc_op_df[sc_op_df[G_VR_LEG_START_TIME] >= evaluation_start_time]
    if evaluation_end_time is not None:
        sc_op_df = sc_op_df[sc_op_df[G_VR_LEG_START_TIME] <= evaluation_end_time]
...
### Whole Week Data
def evaluate_whole_week_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = None, evaluation_end_time = None):   
...
def active_fleet_size_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = None, evaluation_end_time = None):
    ...
    if evaluation_start_time is not None and evaluation_end_time is not None:
        plt.xlim(evaluation_start_time/3600.0, evaluation_end_time/3600.0)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "active_vehicles.png"))
    plt.close()
    return ts, vehs

def dynamic_pricing_factor_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = None, evaluation_end_time = None):
...
def evaluate_dyn_fleetctrl_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = None, evaluation_end_time = None):
    dyn_fleetctrl_dict = {}
    dyn_fleetctrl_dict["active_fleet_size_over_time"] = active_fleet_size_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    dyn_fleetctrl_dict["dynamic_pricing_factor_over_time"] = dynamic_pricing_factor_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    return dyn_fleetctrl_dict
#======

def temporal_evaluation(output_dir, print_comments=False, dir_names_in = {}, evaluation_start_time = None, evaluation_end_time = None):
    ...
    # evaluation interval TODO globals not defined in this branch
    ...
    plot_dict["dyn_fleetctrl_data"] = evaluate_dyn_fleetctrl_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    print(" ... eval week data")
    evaluate_whole_week_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    print(" ... eval day data")
    plot_dict["eval_day_data"] = eval_day_data(output_dir, scenario_parameters, op_attributes, dir_names, op_id, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    print(" ... create fleet stats temporal")
    occupancy_over_time_rel(output_dir, scenario_parameters, op_attributes, dir_names, op_id, bin_size=DEF_TEMPORAL_RESOLUTION, show=False, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    plot_dict["temporal_plots"] = temporal_plots(output_dir, scenario_parameters, op_attributes, dir_names, op_id, show=False, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    plot_dict["occ_stack_plot_data"] = occupancy_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, bin_size=DEF_TEMPORAL_RESOLUTION, show=False, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    vehicle_stats_over_time(output_dir, scenario_parameters, op_attributes, dir_names, op_id, bin_size=DEF_TEMPORAL_RESOLUTION, show=False, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
    print(" ... create rq stats temporal")
    plot_dict["rq_stats_temporal"] = rq_stats_temporal(output_dir, bin_size=DEF_TEMPORAL_RESOLUTION, evaluation_start_time = evaluation_start_time, evaluation_end_time = evaluation_end_time)
