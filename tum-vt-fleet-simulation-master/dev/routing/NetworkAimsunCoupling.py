...
class NetworkAimsunCoupling(NetworkBasicWithStore):
    ...
    def return_travel_costs_1to1(self, origin_position, destination_position, customized_section_cost_function = None):
        ...
        if s is None:
            if self.nodes[origin_node].must_stop() and self.nodes[destination_node].must_stop():
                R = Router(self, origin_node, destination_nodes=self.must_stop_nodes, forward_flag = True, customized_section_cost_function=customized_section_cost_function)
                s_all = R.compute(return_route=False)
                for entry in s_all:
                    cfv, tt, dis = entry[1]
                    if tt < 0:
                        continue
                    dest_node = entry[0][-1]
                    self.travel_time_infos[(origin_node, dest_node)] = (cfv, tt, dis)
