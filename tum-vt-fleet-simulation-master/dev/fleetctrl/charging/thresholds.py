...
from src.misc.globals import G_OP_APS_SOC, G_OP_CHARGE_PUBLIC_ONLY

LOG = logging.getLogger(__name__)


def exchange_low_soc_vehicles_at_depot(fleetctrl, sim_time, soc_threshold):
    ...
    :param soc_threshold: soc threshold
    :return: None
    """
    # deactivate vehicles with SOC below threshold
    reactivate = {}  # (future time, depot_id) -> nr_activate
    found_depots = {}   #depot_id -> depot
    for veh_obj in fleetctrl.sim_vehicles:
        # do not consider inactive vehicles
        if veh_obj.status == 5:
            continue
        current_plan = fleetctrl.veh_plans[veh_obj.vid]
        if current_plan.list_plan_stops:
            last_pstop = current_plan.list_plan_stops[-1]
            if last_pstop.get_charging_power() == 0 and last_pstop.get_planned_arrival_soc() < soc_threshold and \
                    not last_pstop.is_inactive():
                # search depot with replacement vehicle
                dep_rv = fleetctrl.charging_management.find_nearest_depot_replace_veh(last_pstop.get_pos(), fleetctrl.op_id)
                #
                depot, depot_ps = fleetctrl.charging_management.deactivate_vehicle(fleetctrl, veh_obj, sim_time, dep_rv)
                LOG.info(f"Operator {fleetctrl.op_id} sending vehicle {veh_obj.vid} to depot {depot} for charging.")
    ...
    for k, nr_activate in reactivate.items():
        ...
        fleetctrl.charging_management.add_time_triggered_activate(activate_time, fleetctrl.op_id, nr_activate, depot)


class ChargingThresholdDepot(ChargingBase):
